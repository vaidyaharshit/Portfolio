...
import { fetchAll, buildContributionCalendar } from './githubService.js';
import { sendContactMessage, initContactCaptcha, isContactConfigured } from './services/contactService.js';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* -----------------------------------------------------------------------
     19. GITHUB SECTION — LIVE DATA INTEGRATION
         Real data when the API responds; a polished, intentional-looking
         fallback (never a technical error) when it doesn't. Failures start
         a cooldown inside githubService so we never spam the API.
     ----------------------------------------------------------------------- */
  const ghLoading = document.getElementById('githubLoading');
  const ghFallback = document.getElementById('githubFallback');
  ...
  function animateMetricNumber(el, target) {
    if (!el) return;
    if (typeof requestAnimationFrame !== 'function') { el.textContent = target; return; }
    const duration = 800;
  ...
  function showGitHubFallback() {
    ...
    if (ghFallback) ghFallback.style.display = 'flex';
    if (ghData) ghData.style.display = 'none';
  }

  function showGitHubData() {
    if (ghLoading) ghLoading.style.display = 'none';
    if (ghFallback) ghFallback.style.display = 'none';
  ...
  async function initGitHubSection(force = false) {
    if (ghInFlight) return;
    if (ghInitialized && !force) return;
    ghInFlight = true;
    showGitHubLoading();
    try {
      ghAllData = await fetchAll({ force });
      ghInitialized = true;
      ...
      renderYearSelector(ghAllData.availableYears, new Date().getFullYear(), (year) => {
        const cal = buildContributionCalendar(ghAllData.events, year);
      ...
      /* Visitor-facing UI stays polished — details go to the console only. */
      console.warn('GitHub live data deferred:', err && err.code ? err.code : err);
      showGitHubFallback();
    } finally {
      ghInFlight = false;
    }
  ...
  if (ghRetryBtn) {
    ghRetryBtn.addEventListener('click', () => {
      /* Explicit user action: bypass the cooldown for a single attempt. */
      initGitHubSection(true);
    });
  }
  ...
  /* -----------------------------------------------------------------------
     20. CONTACT FORM — real delivery (Web3Forms), validation, retry states
         Success is shown ONLY when the email API confirms the submission.
     ----------------------------------------------------------------------- */
  ...
  if (contactForm && formToast && submitBtn) {
    ...
    /* Fail loudly for the developer BEFORE any visitor submits. */
    const cfgKey = (window.__ENV__ && window.__ENV__.WEB3FORMS_ACCESS_KEY || '').trim();
    if (!cfgKey) {
      console.warn(
        '%c[Contact] SETUP REQUIRED — the form cannot send yet.\n' +
        '%c1. Open https://web3forms.com\n' +
        '2. Enter the inbox address that should receive messages\n' +
        '3. They email you an Access Key\n' +
        '4. Paste it into src/env.js on the WEB3FORMS_ACCESS_KEY line\n' +
        '5. Save + refresh this page',
        'color:#ff2d2d;font-weight:bold', 'color:inherit'
      );
    } else if (/your_web3forms|paste_your|xxxx|^8f2c41a7/i.test(cfgKey)) {
      console.warn('[Contact] src/env.js still contains a PLACEHOLDER or the comment example. Paste your REAL Access Key from the Web3Forms email into the WEB3FORMS_ACCESS_KEY line.');
    } else if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(cfgKey)) {
      console.warn(`[Contact] The key in src/env.js (${cfgKey.length} chars) does not look like a Web3Forms Access Key (expected a UUID like 8f2c41a7-9d3b-...). Double-check the email from Web3Forms.`);
    }
    ...
    const toastTitle = formToast.querySelector('.toast-text strong');
    const toastMsg = formToast.querySelector('.toast-text span');
    ...
    Object.values(fields).forEach(({ input }) => {
      ...
      input.addEventListener('input', () => {
        ...
        const errEl = fields[Object.keys(fields).find(k => fields[k].input === input)]?.error;
    ...
    function validateForm() {
      ...
      const errors = {};
      if (!v.name) errors.name = d.form.errors.nameRequired;
      if (!v.email) errors.email = d.form.errors.emailRequired;
      else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v.email)) errors.email = d.form.errors.emailInvalid;
      if (!v.subject) errors.subject = d.form.errors.subjectRequired;
      if (!v.message) errors.message = d.form.errors.messageRequired;
      return errors;
    }

    function showToast(state) {
      const t = d.toast[state];
      if (toastTimer) clearTimeout(toastTimer);
      if (toastIcon) toastIcon.textContent = t.icon;
      if (toastTitle) toastTitle.textContent = t.title;
      if (toastMsg) toastMsg.textContent = t.message;
      formToast.classList.toggle('toast-error', state === 'error');
      void formToast.offsetWidth; /* restart slide-in animation */
      formToast.classList.add('show');
      toastTimer = setTimeout(() => formToast.classList.remove('show'), state === 'error' ? 9000 : 6000);
    }
    ...
    function setLoading(loading) {
      ...
      submitBtn.innerHTML = loading
        ? `<span>${d.form.sendingText}</span><span class="btn-spinner"></span>`
        : defaultBtnHTML;
    }
    ...
    contactForm.addEventListener('submit', async e => {
      ...
      clearAllErrors(); /* drop stale messages from a previous attempt */
      Object.keys(errors).forEach(key => setFieldError(key, errors[key]));
      const firstInvalid = ['name', 'email', 'subject', 'message'].find(k => errors[k]);
      if (firstInvalid) {
        fields[firstInvalid].input.focus();
        return; /* do not send incomplete forms */
      }

      /* Honeypot: bots that fill it are silently discarded — no send,
         and never a fake success message. */
      ...
      try {
        await sendContactMessage(new FormData(contactForm));
        /* Confirmed by the API response only. */
        contactForm.reset();
        clearAllErrors();
        showToast('success');
      } catch (err) {
        console.error('[Contact] Delivery failed:', err.code || 'UNKNOWN', err.details || err);
        showToast('error'); /* data stays in the form for an easy retry */
      } finally {
        setLoading(false);
      }
    });
